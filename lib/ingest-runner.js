...
async function processJob(job, profileCtx) {
  ...
  const applicationUrl = await resolveApplicationUrl(job, jobText);
  if (applicationUrl !== job.url) {
    const existingResolved = await findPageByUrl(applicationUrl);
    if (existingResolved) {
      return { skipped: true, reason: "already in Notion (resolved URL)" };
    }
  }
  ...
  const { pageId, created } = await upsertJobPage({
    ...
    applicationStatus:
      scoring.match === "Skip"
        ? process.env.NOTION_STATUS_SKIP || "Rejected"
...
async function resolveApplicationUrl(job, jobText) {
  ...
  if (!detectAts(applicationUrl) && isResolveEnabled()) {
    try {
      const resolved = await resolveApplyUrl(applicationUrl, {
        description: job.description || jobText,
        source: job.source,
      });
